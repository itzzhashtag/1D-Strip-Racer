...
#define RAINBOW_STEP    512       // Hue increment per idle tick

// How long with NO button presses during a race before defeat tone
#define RACING_INACTIVITY_MS  60000UL   // 1 minute

// Duration of each countdown beat (3 / 2 / 1 / GO) in ms
// All beats are equal — like a real traffic light
#define COUNTDOWN_BEAT_MS     900
...
unsigned long lastButtonPressMs = 0;   // Updated on every button press during race

// 7-segment helpers
static const uint8_t SEG_BLANK = 0b00000000;
static const uint8_t SEG_DASH  = 0b01000000;
// "dEAd" — shown when defeat tone plays
...
// tud tu du  tud du duu  … ding (C5 = 523 Hz — low, resolving note)
const int WIN_NOTES[] = {
    2637, 2637, 0, 2637, 0, 2093, 2637, 0, 3136,   // main phrase
    0, 0, 523                                        // rest, rest, DING ↓
};
const int WIN_NOTES_LEN = sizeof(WIN_NOTES) / sizeof(int);

// Defeat / sad descending tune
...
/** True only on the tick a button transitions released → pressed. */
...
/** Show a single count digit centred: " N N " */
...
void displayWinner(int winner) {
    ...
    uint8_t winDigit = (NUM_PLAYERS == 2 && winner == 1) ? 3 : (uint8_t)winner;
    for (int f = 0; f < 5; f++) {
...
// ================================================================
//  DEFEAT TONE  (called from racing inactivity timeout)
// ================================================================
...
// ================================================================
//  IDLE STATE  — rainbow only, no timeout here
// ================================================================
...
void loopIdle() {
    // Rainbow — no timeout; defeat tone only happens during a race
...
// ================================================================
//  COUNTDOWN — symmetric sweep from both sides, equal beat timing
// ================================================================

/*
 * The LED strip forms a CIRCLE.  LED 0 is the start/finish line.
 * The midpoint (NUM_LEDS/2) is the opposite side of the circle.
 *
 * For each beat a lit pixel starts at the midpoint and sweeps
 * toward LED 0 from BOTH directions simultaneously — one pixel
 * going left (decreasing index) and one going right (increasing
 * index, wrapping), so both approach the start line together.
 *
 *         mid                     mid
 *          ↓                       ↓
 *  ───→→→[60]←←←────────────────[60]→→→←←←───
 *        both pixels close in on LED 0 from both sides
 *
 * durationMs: total time for the sweep (same for every beat)
 */
void symmetricSweep(uint32_t color, int durationMs) {
    int half        = NUM_LEDS / 2;
    int delayPerLed = max(1, durationMs / half);   // even division

    for (int step = 0; step <= half; step++) {

        // Left pixel: midpoint → 0  (decreasing)
        int leftPos  = half - step;
        // Right pixel: midpoint → NUM_LEDS-1 → 0  (increasing, wraps)
        int rightPos = (half + step) % NUM_LEDS;

        strip.clear();

        // Main pixels
        strip.setPixelColor(leftPos,  color);
        strip.setPixelColor(rightPos, color);

        // Trailing tail — left side goes +1, +2 (away from 0)
        int lt1 = leftPos  + 1;
        int lt2 = leftPos  + 2;
        // Trailing tail — right side goes -1, -2 (wrapping)
        int rt1 = (rightPos - 1 + NUM_LEDS) % NUM_LEDS;
        int rt2 = (rightPos - 2 + NUM_LEDS) % NUM_LEDS;

        if (lt1 <= half) strip.setPixelColor(lt1, dimColor(color, 0.35f));
        if (lt2 <= half) strip.setPixelColor(lt2, dimColor(color, 0.12f));
        strip.setPixelColor(rt1, dimColor(color, 0.35f));
        strip.setPixelColor(rt2, dimColor(color, 0.12f));
...
void runCountdown() {
    ...
    // Each beat: number shown, sweep color, beep pitch
    // durationMs is the same for all → equal real-life timing
    struct Beat {
        int      count;
        uint32_t color;
        int      beepFreq;
    ...
    for (int b = 0; b < 3; b++) {
        ...
        // Symmetric sweep — fills exactly COUNTDOWN_BEAT_MS
        symmetricSweep(beats[b].color, COUNTDOWN_BEAT_MS);

        // Short beep at the end of each sweep
    ...
    // Instant full-strip white flash for COUNTDOWN_BEAT_MS
    for (int i = 0; i < NUM_LEDS; i++) strip.setPixelColor(i, 0x00FFFFFF);
    strip.show();
    tone(PIN_AUDIO, 1800, 220);
    delay(COUNTDOWN_BEAT_MS);
    noTone(PIN_AUDIO);
    strip.clear();
    strip.show();

    // Reset all player state; start the race clock
    for (int i = 0; i < NUM_PLAYERS; i++) {
        players[i] = {0.0f, 0.0f, 0, false, false};
    }
    lastButtonPressMs = millis();   // Inactivity clock starts now
    updateDisplay();
...
void runWinAnimation(int winner) {
    ...
    // Mario course-clear jingle — ends with a low resolving DING (C5)
...
void loopRacing() {
    ...
    // ── Inactivity check ─────────────────────────────────────────
    // If nobody has pressed anything for RACING_INACTIVITY_MS → defeat
    if (millis() - lastButtonPressMs >= RACING_INACTIVITY_MS) {
        Serial.println("[RACE] Inactivity timeout — defeat");
        playDefeatSequence();
        enterIdle();
        return;
    }

    // ── Physics for each player ──────────────────────────────────
    for (int i = 0; i < NUM_PLAYERS; i++) {
...
void setup() {
    Serial.begin(115200);
    Serial.println("[BOOT] LED Race v4");
